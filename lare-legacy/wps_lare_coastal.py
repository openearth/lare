...
# http://localhost:5000/wps?service=wps&request=Execute&version=2.0.0&Identifier=lare_coastal&datainputs=sessionid=1773324234723672
#  
...
# local
from .lare_coastal import mainhandler_coastal

class WpsLareCoastal(Process):
	...
	def __init__(self):
		...
		inputs = [
			LiteralInput(
                ...
                data_type='string',
                keywords=['session ID', 'LARE process']
            )]
		...
		super(WpsLareCoastal, self).__init__(
		    self._handler,
		    identifier='lare_coastal',
		    version='1.0',
		    title='Coastal LARE Process',
		    abstract='This process enables selection of a coastal region and returns the suggested size of the unit of measurement in m2' \
			         'The process is carried out for a selected coastal region',
		    profile='',
		    metadata=[Metadata('WpsLareCoastal'), Metadata('lare/coastal')],
		    inputs=inputs,
		    outputs=outputs,
	...
	def _handler(self, request, response):
		...
		try:		
			...
			logging.info(f'!-- wps lare coastal, create dataframe sessionid {sessionid}')

			res = mainhandler_coastal(sessionid)
